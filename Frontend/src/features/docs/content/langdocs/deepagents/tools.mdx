...
Return a string for text-only results, or an ordered list of content blocks for text plus media or interleaved multimodal output. See [Multimodal](/oss/deepagents/multimodal) and Tool return values for examples and context-compression considerations.
