...
product: deepagents
resource: /docs/deepagents
source: /oss/deepagents/overview
tags:
  - deepagents
  - overview
timestamp: 2026-08-13T16:34:53Z
code_examples:
  - ../code/deepagents/overview-quickstart.py
  - ../code/deepagents/overview-quickstart.ts
  - ../code/deepagents/overview-tools.py
  - ../code/deepagents/frontend-todo-list-setup.py
  - ../code/deepagents/frontend-todo-list-setup.ts
---
...
**Python**
Code example: [`code/deepagents/overview-quickstart.py`](../code/deepagents/overview-quickstart.py)
...
Code example: [`code/deepagents/overview-quickstart.ts`](../code/deepagents/overview-quickstart.ts)
...
Pass custom functions, LangChain tools, or tools from any [MCP server](../deepagents/tools.md#mcp-tools) with the `tools=` parameter. Deep Agents fully support the Model Context Protocol (MCP), letting you connect to databases, APIs, file systems, and more through a standard interface.

**Python**
Code example: [`code/deepagents/overview-tools.py`](../code/deepagents/overview-tools.py)
...
```
    ...
    Removing `FilesystemMiddleware` itself via `excluded_middleware` is intentionally rejected—it is required scaffolding in the Deep Agents stack. Use `excluded_tools` to hide only the model-visible tool surface and leave the middleware in place. To remove the `task` tool, see [Running without subagents](../deepagents/subagents.md#running-without-subagents).
...
Permissions do not apply to [sandbox backends](../deepagents/sandboxes.md), which support arbitrary command execution via the `execute` tool. For custom validation logic, use backend policy hooks.
...
For sandbox setup, providers, and file transfer APIs, see [Sandboxes](../deepagents/sandboxes.md). For the QuickJS runtime and programmatic tool calling, see Interpreters.
...
For configuration details, see Context engineering. For multimodal inputs and tool outputs, see [Multimodal](../deepagents/multimodality.md).
...
**Python**
Code example: [`code/deepagents/frontend-todo-list-setup.py`](../code/deepagents/frontend-todo-list-setup.py)
...
**JavaScript / TypeScript**
Code example: [`code/deepagents/frontend-todo-list-setup.ts`](../code/deepagents/frontend-todo-list-setup.ts)
...
For configuration options and behavior details, see [To-do list](../langchain/middleware.md#to-do-list).
...
**Running without subagents (no `task` tool)**
    To run an agent without the `task` tool, see [Running without subagents](../deepagents/subagents.md#running-without-subagents). Do not try removing `SubAgentMiddleware` via `excluded_middleware`—that is intentionally rejected. Instead, disable the auto-added subagent via the harness profile and pass no synchronous subagents via `subagents=`. Async subagents are unaffected. See the full stack for the complete ordering.
